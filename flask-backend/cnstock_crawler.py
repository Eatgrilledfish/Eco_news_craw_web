...
class MyThread(threading.Thread):
    ...
    def spider(self):
        while not self.url_queue.empty() and not self.shutdown_event.is_set():
            ...
            if self.check_url(url):
                ...
                if MyThread.processed_count >= 100:
                    logger.info('Reached 100 URLs, shutting down.')
    ...
    def process_url(self, url):
        try:
            ...
            title = html.xpath('////*[contains(@id,"pager-content")]/div[contains(@class,"content")]/p/text()')
            results = html.xpath('////*[contains(@id,"pager-content")]')

            # print("this is the result",title)
            # print('this is titlee' ,title)
            for result in results:
                item['url'] = url
                item['title'] = result.xpath('./h1/text()')[0]
                item['publish_time'] = result.xpath('./div[contains(@class,"bullet")]/span[1]/text()')[0]
